  ...
  return (
    ...
    >
      <div className={`flip-card-inner ${isFlipped ? 'flipped' : ''}`}>
        {/* Front of card */}
        <div className="flip-card-front bg-card shadow-xl rounded-xl overflow-hidden">
          ...
          <div className="p-6">
            <h3 className="font-bold text-xl text-card-foreground mb-2">{deal.destination}</h3>
            <div className="flex items-center mb-2">
              {Array.from({ length: deal.hotelRating }).map((_, i) => (
                <i key={i} className="fas fa-star text-foreground"></i>
              ))}
              <span className="ml-2 text-muted-foreground">{deal.hotelRating} Stars</span>
            </div>
            <div className="flex items-center justify-between">
              <div>
                <span className="text-3xl font-bold text-primary">₹{deal.price}</span>
                <span className="text-muted-foreground line-through ml-2">₹{deal.originalPrice}</span>
              </div>
        ...
        {/* Back of card */}
        <div className="flip-card-back bg-primary text-primary-foreground p-6 flex flex-col justify-between rounded-xl">
          <div>
          ...
          <div className="space-y-2 mt-4">
            <Button
              ...
              className="w-full bg-primary-foreground text-primary font-semibold py-2 rounded-lg hover:bg-primary-foreground/90 transition-colors"
            >
            ...
            <Button
              ...
              className="w-full bg-secondary text-secondary-foreground font-semibold py-2 rounded-lg hover:bg-secondary/80 transition-colors"
            >
